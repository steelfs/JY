...
public class PlayerBase : MonoBehaviour
{
    ...
    protected virtual void Initialize()
    {
        if (!isInitialized)
        {
            ...
            for (int i = 0; i < shipTypeCount; i++)
            {
                ...
                board.onShipAttacked[shipType] = () => opponent.successAttackCount++;
    ...
    {
        if (!IsActionDone && Board.IsInBoard(attackGridPos) && opponent.Board.IsAttackable(attackGridPos))  // 행동을 안했rh, 공격지점이 보드 안이고, 공격 가능한 지점일 때
        {
    ...
    private void AddHigh(int index)
    {
        if( !attackHighIndices.Contains(index) )   // 안들어 있을 때만 추가
        {
            ...
            obj.gameObject.SetActive(GameManager.Inst.IsTestMode);
