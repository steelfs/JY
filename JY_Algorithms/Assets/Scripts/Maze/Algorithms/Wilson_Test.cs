...
public class Wilson_Test : MazeGenerator_Test
{
    ...
    public override async void MakeMaze()
    {
        ...
        Dictionary<WilsonCell_Test, WilsonCell_Test> confirmedSet = new Dictionary<WilsonCell_Test, WilsonCell_Test> ();//탐색속도를 높이기 위해 추가한 것인데 이곳에서 굳이 Dictionary를 쓰는 것은 과했다. 
        ...
        Stack<WilsonCell_Test> path = new Stack<WilsonCell_Test> ();//움직이는 경로를 저장할 스택
        Dictionary<WilsonCell_Test, WilsonCell_Test> pathSet = new Dictionary<WilsonCell_Test, WilsonCell_Test> ();//셀의 갯수가 몇개 없기때문에 검색속도는 별 차이 없을 것. 이것 역시 너무 과했다.


        WilsonCell_Test first = cells[UnityEngine.Random.Range(0, cells.Length)] as WilsonCell_Test;//처음 도착지점 설정
        ...
        WilsonCell_Test start = notConfirmed[0];// 위에서 Shuffle로 섞어 놓았기 때문에 하나씩 뽑아 써도 랜덤한 셀을 가져오게 된다.
        path.Push(start);//경로 스텍에 추가
        pathSet.Add(start, start);

        while(notConfirmed.Count > 0)
        {
            WilsonCell_Test current = path.Peek();//경로 중 가장 최근에 추가된것은 기준으로
            WilsonCell_Test[] neighbors = GetNeighbors(current)as WilsonCell_Test[];// 상하좌우 셀을 가져온다.(이전 셀과 보드 바깥쪽은 제외)
            WilsonCell_Test next = null;//가져온 셀들 중 랜덤하게 하나를 다음셀로 설정
            while(next == null)
            {
            ...
            else if (confirmedSet.ContainsKey(next))//도착지에 도착했을 경우
            {
                ...
                foreach (WilsonCell_Test picked in path)//path에 추가되어있던 셀들을 확정 리스트에 추가시키면서 머티얼도 바꿔준다.
                {
