...
public class Kruskal : MazeGenerator_Test
{
    Kruskal_Cell previousFrom;//머티리얼 변경용
    Kruskal_Cell previousTo;
    UnionFind unionFind;
    public Action on_DoneWithMakeMaze;
    ...
    public override async void MakeMaze()
    {
        ...
        {
            int index = UnityEngine.Random.Range(0, remainCells.Count);
        ...
        on_Set_DefaultMaterial?.Invoke(GridToIndex(previousFrom.X, previousFrom.Y));
        on_Set_DefaultMaterial?.Invoke(GridToIndex(previousTo.X, previousTo.Y));
        on_DoneWithMakeMaze?.Invoke();
